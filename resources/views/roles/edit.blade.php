        ...
        <div class="bg-white rounded-lg shadow relative dark:bg-gray-800 dark:text-white">
            ...
            <div class="p-6 space-y-6">
                {!! Form::model($role, ['method' => 'PATCH','route' => ['roles.update', $role->id]]) !!}
                    ...
                    <strong for="full_name" class="text-sm font-medium text-gray-900 block mb-2 dark:text-white">Permissions</strong>
                    <div class="grid grid-cols-4 gap-3 mb-2">
                        @foreach($permission as $value)
                            <label>{{ Form::checkbox('permission[]', $value->id, in_array($value->id, $rolePermissions) ? true : false, array('class' => 'w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600')) }}
                            {{ $value->name }}</label>
                        <br/>
                        @endforeach
                    </div>
