<div>
    <div class="p-4 bg-white block sm:flex items-center justify-between border-b border-gray-200 lg:mt-1.5">
        <div class="mb-1 w-full">
            <div class="sm:flex">
                <div class="mb-4">
                  <h1 class="text-xl sm:text-2xl font-semibold text-gray-900">Utility Providers Management</h1>
                </div>
                <div class="flex items-center space-x-2 sm:space-x-3 ml-auto">
                  @can('provider-create')
                    <button type="button" wire:click="toggleModal" class="w-1/2 text-gray-900 bg-teal-500 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium inline-flex items-center justify-center rounded-lg text-sm px-3 py-2 text-center sm:w-auto">
                        <svg class="-ml-1 mr-2 h-6 w-6" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z" clip-rule="evenodd"></path></svg>
                        Add utility provider
                    </button>
                  @endcan
                  
                  @if ($showUPCreateModel)
                    @livewire('utility-provider-create-model')
                  @endif
                  {{-- @livewire('utility-provider-create-model') --}}
                </div>
                          ...
                          @foreach ($providers as $key => $provider)
                            <tr class="">
                                ...
                                <td class="">
                                  ...
                                  @endcan
                                  @can('provider-edit')
                                      <button type="button" data-modal-toggle="edit-up-modal{{$provider['id']}}" class="text-white bg-blue-500 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm inline-flex items-center px-3 py-2 text-center">
                                  ...
                                  @include('utility_provider.modal.edit')
                                </td>
                            </tr>
                          @endforeach
                        </tbody>
